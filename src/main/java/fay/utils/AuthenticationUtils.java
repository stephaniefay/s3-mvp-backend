...
public class AuthenticationUtils {
    ...
    public RestResponse<UserResponse> createUser(CreateUserBody body) {
        ...
        UserResponse response = repository.createUser(body);
        Optional<User> optional = repository.getUser(response.getId());

        if (optional.isPresent()) {
            String token = generateToken(optional.get());
    ...
    public String validateUserAndGenerateToken(LoginBody body) {
        Optional<User> user = repository.getUserByUsernameOrEmail(body.getUsername());
